...
// Lists candidate message IDs only (cheap, no body fetch) so callers can
// filter out already-processed messages before paying for detail fetches.
export async function listOutlookJobMessageIds(accessToken, sinceDate) {
  const headers = {
...
// Fetches full details for a specific set of message IDs.
export async function getOutlookJobEmailDetails(accessToken, messageIds) {
  const headers = {
    Authorization: `Bearer ${accessToken}`,
    ConsistencyLevel: "eventual",
  };
    ...
    messageIds.map(async (id) => {
      try {
        ...
        return {
          ...
          body: (body || data.bodyPreview || "").slice(0, 2500),
        };
      } catch (_) {
        return null;
      }
