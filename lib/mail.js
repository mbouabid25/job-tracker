...
const MAX_PER_SYNC = 50; // small enough to classify + retire within Vercel's 60s limit; auto-continue handles the rest
...
// Returns { emails, hasMore } where hasMore=true means there are still unseen
// emails beyond this batch — the caller should trigger another sync.
export async function fetchJobEmailsForUser(ownerId, sinceDate) {
  ...
  for (const account of accounts) {
    ...
    try {
      ...
      const unseenAll = candidateIds.filter((id) => !alreadySeen.has(id));

      if (unseenAll.length > MAX_PER_SYNC) hasMore = true;
      const unseenIds = unseenAll.slice(0, MAX_PER_SYNC);
