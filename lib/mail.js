...
async function getFreshToken(account) {
  ...
  updateMailAccountTokens(account.id, {
...
export async function fetchJobEmailsForUser(ownerId, sinceDate) {
  const accounts = listMailAccounts(ownerId);
  if (!accounts.length) {
  ...
  for (const account of accounts) {
    ...
    try {
      ...
      // Filter out emails Claude has already processed for this user
      const ids = emails.map((e) => e.id).filter(Boolean);
      const alreadySeen = filterUnseenEmails(ownerId, ids);
...
export function markEmailsAsProcessed(ownerId, emails) {
  const ids = emails.map((e) => e.id).filter(Boolean);
  markEmailsProcessed(ownerId, ids);
}
