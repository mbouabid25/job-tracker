...
export async function listJobMessageIds(accessToken, sinceDate) {
  ...
  // Keep listing fast: 3 pages × 500 = 1500 ids per query is plenty for a
  // single sync pass. auto-continue handles what this pass doesn't reach.
  const MAX_PAGES_PER_QUERY = Number(process.env.JOB_MAX_PAGES || 3);
  const MAX_TOTAL_IDS = Number(process.env.JOB_MAX_IDS || 2000);
