...
export async function initDb() {
  ...
  // applied_at: when the application went in. stage_at: when it reached its
  // current status. last_updated can stand in for neither — every newer
  // email overwrites it.
  try { await db.execute("ALTER TABLE jobs ADD COLUMN applied_at TEXT"); } catch (_) {}
  try { await db.execute("ALTER TABLE jobs ADD COLUMN stage_at TEXT"); } catch (_) {}
  // Rows from before these columns: an 'applied' row's latest email is the
  // application itself; any other row's latest email is what moved it.
...
// When the application went in, as far as this batch of emails can tell.
function incomingAppliedAt(j) {
  return j.appliedAt || (j.status === "applied" ? j.lastUpdated : null) || null;
}
...
export async function upsertJobs(userId, jobs) {
  ...
  for (const j of jobs) {
    ...
    let existing = await db.execute({
      sql: "SELECT id, company, position, status, last_updated, recruiter, notes, applied_at, stage_at FROM jobs WHERE user_id = ? AND job_key = ?",
      args: [userId, key],
    });
    ...
    if (!existing.rows.length) {
      const candidates = await db.execute({
        sql: "SELECT id, company, position, status, last_updated, recruiter, notes, applied_at, stage_at FROM jobs WHERE user_id = ?",
        args: [userId],
      });
    ...
    if (existing.rows.length) {
      ...
      const appliedAt = earliest(cur.applied_at, incomingAppliedAt(j));
      // stage_at moves only when this email is what changed the status.
      const stageAt = status !== cur.status
        ? (j.stageAt || j.lastUpdated || new Date().toISOString())
        : cur.stage_at;
      ...
      await db.execute({
        sql: `INSERT INTO jobs (user_id, company, position, recruiter, status, last_updated, notes, job_key, source_id, applied_at, stage_at)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        args: [userId, j.company, j.position, j.recruiter ?? null, j.status, j.lastUpdated ?? null, j.notes ?? null, key,
               ...
               incomingAppliedAt(j), j.status === "applied" ? null : (j.stageAt || j.lastUpdated || null)],
      });
      stats.inserted += 1;
    }
  }
...
export async function updateJobStatus(userId, jobId, status) {
  await ensureInit();
  await getDb().execute({
    sql: "UPDATE jobs SET stage_at = CASE WHEN status <> ? THEN ? ELSE stage_at END, status = ?, updated_at = datetime('now') WHERE user_id = ? AND id = ?",
    args: [status, new Date().toISOString(), status, userId, jobId],
  });
}
