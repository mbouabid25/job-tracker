process.env.JOB_LOOKBACK_DAYS = process.env.JOB_LOOKBACK_DAYS || '365';
...
const gsrc = fs.readFileSync('./lib/gmail.js','utf8');
const qblock = gsrc.slice(gsrc.indexOf('const JOB_QUERIES = ['), gsrc.indexOf('];', gsrc.indexOf('const JOB_QUERIES = ['))+2);
const SIGNAL = eval(qblock + '; JOB_QUERIES');
...
console.log('all candidates (full query set):', ids.size);
